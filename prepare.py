...
def savenpy_luna(id, annos, filelist, luna_segment, luna_data, savepath):
    ...
    if isClean:
        ...
        # plt.imshow(sliceim[90], cmap=plt.cm.gray)
        # plt.show()
    ...
    if not os.path.exists(finished_flag):
        ...
        for d in subsetdirs:
            ...
            for f in files:
                ...
                filename = '0' * (3 - len(str(id))) + str(id) #三位数字表示文件名，不足补0
                shutil.move(os.path.join(d, f), os.path.join(luna_data, filename + f[-4:]))#f[-4:]取后缀.raw/.mhd
        ...
        for file in files:
            with open(os.path.join(luna_data, file), 'r') as f:
                ...
                print(content[-1])
                lunafull.append(filename)
            with open(os.path.join(luna_data, file), 'w') as f:
                f.writelines(content)
        luna_full = np.array(lunafull, dtype='S32')
        np.save(("train_luna08.npy"), luna_full)
