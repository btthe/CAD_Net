...
class Loss(nn.Module):
    ...
    def forward(self, output, labels, train=True):
        ...
        if len(pos_output) > 0:
            ...
            # metrics_output.extend(pos_prob.data.cpu().numpy().tolist())
            # metrics_label.extend( pos_labels[:, 0].data.cpu().numpy().tolist())
        else:
...
class GetPBB(object):
    ...
    def __call__(self, output, thresh=-3, ismask=False):
        ...
        output_size = output.shape #72,72,72,3,5
        ...
        prob = torch.from_numpy(output[..., 0])
        prob = get_prob(prob)
        prob = prob.numpy()
        ...
        prob = prob[xx, yy, zz, aa]
        prob = prob.reshape(outputshape[0],1)
        output = np.hstack((prob,output))
