...
class DataBowl3Detector(Dataset):
            ...
            for i, l in enumerate(labels):
                if len(l) > 0:
                    for t in l: #t[3]结节直径
                        ...
                        if t[3] > sizelim2:
                            self.bboxes += [[np.concatenate([[i], t])]] * 2 #数据增强，2倍
                        if t[3] > sizelim3:
                            self.bboxes += [[np.concatenate([[i], t])]] * 4
            self.bboxes = np.concatenate(self.bboxes, axis=0)

        self.crop = Crop(config) #Crop类的实例对象
    ...
    def __getitem__(self, idx, split=None):
        ...
        isRandomImg = False
        if self.phase != 'test':
            if idx >= len(self.bboxes): #正样本裁剪完后为true
                isRandom = True
        ...
        if self.phase != 'test':
            ...
            else: # randomly select image
                randimid = np.random.randint(len(self.lunanames)) #kagglenames改为lunanames
                filename = self.lunanames[randimid]
                imgs = np.load(filename)
                bboxes = self.sample_bboxes[randimid]
                isScale = self.augtype['scale'] and (self.phase == 'train')
                sample, target, bboxes, coord = self.crop(imgs, [], bboxes, isScale=False, isRand=True)
            label = self.label_mapping(sample.shape[1:], target, bboxes) #32*32*32*3*5
...
class Crop(object):
    ...
    def __call__(self, imgs, target, bboxes, isScale=False, isRand=False): #__call__()的作用是使对象实例能够像函数一样被调用 即直接调用crop()而不是Crop()
        if isScale:
            radiusLim = [3., 35.]  #半径限制  要改
            scaleLim = [0.75, 1.25]
        ...
        xx, yy, zz = np.meshgrid(np.linspace(normstart[0], normstart[0] + normsize[0], self.crop_size[0] // self.stride),
                                 ...
                                 indexing='ij')
        coord = np.concatenate([xx[np.newaxis, ...], yy[np.newaxis, ...], zz[np.newaxis, :]], 0).astype('float32') #shape 32 32 32
        ...
        if isScale:
            with warnings.catch_warnings():
                ...
                crop = zoom(crop, [1, scale, scale, scale], order=1)  #shape规范为1，128，128，128
...
###标签映射函数与Faster-RCNN非常相似 32x32x32x3x5 这五列代表[概率，dz, dx, dy, dr]
class LabelMapping(object):
    ...
    def __call__(self, input_size, target, bboxes):
        ...
        th_pos = self.th_pos
        # struct = generate_binary_structure(3, 1) 不同于data_detector

        output_size = []
        for i in range(3):
            assert (input_size[i] % stride == 0)
            output_size.append(int(input_size[i] / stride)) #32

        label = -1 * np.ones(output_size + [len(anchors), 5], np.float32)#32,32,32,3,5
        ...
        for bbox in bboxes:
            for i, anchor in enumerate(anchors):
                ...
                # label[iz, ih, iw, i, 0] = 1  不同于data_detector
        ...
        if self.phase == 'train' and self.num_neg > 0:
            ...
            label[:, :, :, :, 0] = 0
            label[neg_z, neg_h, neg_w, neg_a, 0] = -1   #最后五维的第一个元素为-1 标签 负样本？
...
def select_samples(bbox, anchor, th, oz, oh, ow):
    ...
    min_overlap = np.power(max(d, anchor), 3) * th / max_overlap / max_overlap  #numpy.power()数组元素求n次方
    if min_overlap > max_overlap:
        ...
        iz = iz.reshape((-1, 1, 1)) #5,1,1
        ih = ih.reshape((1, -1, 1)) #1,6,1
        iw = iw.reshape((1, 1, -1)) #1,1,6
        iz = np.tile(iz, (1, lh, lw)).reshape((-1))  #np.tile 数组沿各个方向复制
