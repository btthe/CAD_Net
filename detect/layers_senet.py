...
class PostRes(nn.Module):
    def __init__(self, n_in, n_out, stride=1):
        ...
        self.globalAvgPool2 = nn.AvgPool3d(kernel_size=3)
        self.fc1 = nn.Linear(in_features=n_out, out_features=round(n_out / 8))
        self.fc2 = nn.Linear(in_features=round(n_out / 8), out_features=n_out)
        self.sigmoid = nn.Sigmoid()
        self.convfc1=  nn.Conv3d(n_out,round(n_out / 8), kernel_size=1, stride=1, padding=0)
        self.convfc2 =  nn.Conv3d(round(n_out / 8), n_out, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        original_out = out #2,32,64,64,64
        out = self.globalAvgPool(out) #2,32,1,1,1
        # out = out.view(out.size(0), -1)#2,3764768
        # out = self.fc1(out)
        # out = self.relu(out)
        # out = self.fc2(out)
...
def hard_mining(neg_output, neg_labels, num_hard):
    _, idcs = torch.topk(neg_output, min(num_hard, len(neg_output))) #返回前k个最大元素
...
class Loss(nn.Module):
    def __init__(self, num_hard=0):
        ...
        self.classify_loss = nn.BCELoss()  #单目标二分类交叉熵函数Binary_Corss_Entropy
        self.regress_loss = nn.SmoothL1Loss() #适用于计算差的绝对值
        self.num_hard = num_hard

    def forward(self, output, labels, train=True):
        batch_size = labels.size(0) #[1,32,32,32,3,5]
        output = output.view(-1, 5) #[..,5]
        labels = labels.view(-1, 5) #[..,5]

        pos_idcs = labels[:, 0] > 0.5  #阈值
        pos_idcs = pos_idcs.unsqueeze(1).expand(pos_idcs.size(0), 5)
        pos_output = output[pos_idcs].view(-1, 5)  #[1,5]
...
#非极大值抑制Non-Maximum Suppression：NMS 对检测得到的全部 boxes 进行局部的最大搜索，
# 以搜索某邻域范围内的最大值，从而滤出一部分 boxes，提升最终的检测精度.选出最优的box
...
def acc(pbb, lbb, conf_th, nms_th, detect_th):  #test中的lbb只有正样本？
    ...
    for p in pbb:
        ...
        for i, l in enumerate(lbb):
            score = iou(p[1:5], l)
            if score > bestscore:  #找出重合度最大值
                bestscore = score
                besti = i
        if bestscore > detect_th: #detect_th设定的重合度阈值
            flag = 1
            if l_flag[besti] == 0:
                l_flag[besti] = 1
