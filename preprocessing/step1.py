...
def load_scan(path):
    ...
    if slices[0].ImagePositionPatient[2] == slices[1].ImagePositionPatient[2]:
        ...
        while slices[0].ImagePositionPatient[2] == slices[sec_num].ImagePositionPatient[2]:
            sec_num = sec_num + 1
        slice_num = int(len(slices) / sec_num) #切片的数量
        slices.sort(key=lambda x: float(x.InstanceNumber))
        slices = slices[0:slice_num]
        slices.sort(key=lambda x: float(x.ImagePositionPatient[2]))
    try:
        slice_thickness = np.abs(slices[0].ImagePositionPatient[2] - slices[1].ImagePositionPatient[2]) #可能发生错误的代码
    except:
        slice_thickness = np.abs(slices[0].SliceLocation - slices[1].SliceLocation) # 如果出现异常执行的代码
...
def get_pixels_hu(slices):
    ...
    image = image.astype(np.int16) # astype：转换数组的数据类型。

    # Convert to Hounsfield units (HU) # 将扫描图像的像素值转换为Hounsfield单位
    ...
    return np.array(image, dtype=np.int16), np.array([slices[0].SliceThickness] + list(slices[0].PixelSpacing),
                                                     dtype=np.float32) # 转换后的图像数据和相关信息（如切片厚度和像素间距）
    # 作为numpy数组返回


def binarize_per_slice(image, spacing, intensity_th=-600, sigma=1, area_th=30, eccen_th=0.99, bg_patch_size=10):
    bw = np.zeros(image.shape, dtype=bool) # 生成空图像

    # prepare a mask, with all corner values set to nan遮罩
...
def two_lung_only(bw, spacing, max_iter=22, max_ratio=4.8):
    def extract_main(bw, cover=0.95):
        for i in range(bw.shape[0]):
            ...
            properties = measure.regionprops(label) # 连通区域标记
            ...
            while sum < np.sum(area) * cover:
                sum = sum + area[count]
                count = count + 1
            filter = np.zeros(current_slice.shape, dtype=bool) #凸包图像给filter
...
if __name__ == '__main__':
    INPUT_FOLDER = 'C:/Users/chris/Desktop/test/edge cases/'
    patients = os.listdir(INPUT_FOLDER)
    patients.sort()
    print(patients)
    case_pixels, m1, m2, spacing = step1_python(os.path.join(INPUT_FOLDER, patients[1]))
    plt.imshow(m1[90])
    plt.imshow(m2[90])
    plt.show()
